...
mod tests {
    ...
    fn loads_real_schema_rows_with_continuations_and_token_buckets() {
        ...
        let entries = load_from_fixture_with_pricing(&fixture, true, &snapshot_pricing());
    ...
    fn attributes_step_and_retry_usage_to_their_token_models() {
        ...
        // The embedded pricing snapshot omits Gemini models, so price the two
        // models explicitly to keep the attribution check self-contained.
        let mut pricing = PricingMap::default();
        assert_eq!(
            pricing.load_json(
                r#"{
            ...
            .map(|kind| {
                ...
                (
                    ...
                    "table": rows.iter().map(|row| json!({
                        "firstColumn": turbotokens_core::first_column(kind),
                        "period": turbotokens_core::summary_period(row),
