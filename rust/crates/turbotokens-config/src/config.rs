...
fn config_error(message: impl Into<String>) -> String {
    format!("Invalid turbotokens config: {}", message.into())
}
...
fn reserved_named_pi_store_names() -> Vec<&'static str> {
    std::iter::once("all")
        .chain(turbotokens_core::BUILT_IN_AGENT_NAMES.iter().copied())
        .collect()
}
...
fn discover_config_paths() -> Vec<PathBuf> {
    ...
    if let Ok(cwd) = env::current_dir() {
        paths.push(cwd.join(".turbotokens").join("turbotokens.json"));
    }
    paths.extend(
        claude_config_dirs()
            .into_iter()
            .map(|dir| dir.join("turbotokens.json")),
    );
    paths
}
...
fn claude_config_dirs() -> Vec<PathBuf> {
    ...
    turbotokens_core::home::home_dir()
...
fn is_agent_command(command: &str) -> bool {
    turbotokens_core::BUILT_IN_AGENT_NAMES.contains(&command)
}
...
impl From<ConfigCostSource> for CostSource {
    fn from(value: ConfigCostSource) -> Self {
        match value {
            ...
            ConfigCostSource::Turbotokens => Self::Turbotokens,
...
mod tests {
    ...
    use turbotokens_core::DEFAULT_SESSION_DURATION_HOURS;
    ...
    fn applies_schema_backed_report_specific_options() {
        ...
        assert_eq!(statusline.cost_source, turbotokens_cli::CostSource::Both);
    ...
    fn rejects_named_pi_store_bad_shape() {
        ...
        assert_eq!(
            config.config_error(),
            Some(
                "Invalid turbotokens config: pi.stores[0] must contain string fields 'name' and 'path'"
            )
        );
    }
    ...
    fn rejects_named_pi_store_builtin_agent_collision() {
        ...
        assert_eq!(
            config.config_error(),
            Some("Invalid turbotokens config: pi.stores name 'pi' collides with a built-in agent")
        );
    }
    ...
    fn rejects_named_pi_store_duplicate_name() {
        ...
        assert_eq!(
            config.config_error(),
            Some("Invalid turbotokens config: duplicate pi.stores name 'omp'")
        );
    }
