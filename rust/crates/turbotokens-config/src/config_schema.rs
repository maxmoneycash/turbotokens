...
pub fn generate_config_schema_json() -> String {
    let generator = SchemaSettings::draft07()
        ...
        .into_generator();
    let mut schema =
        serde_json::to_value(generator.into_root_schema_for::<TurbotokensConfig>()).unwrap();
    if let Value::Object(root) = &mut schema {
        root.insert(
            "title".to_string(),
            Value::String("turbotokens Configuration".to_string()),
        );
        root.insert(
            "description".to_string(),
            Value::String("Configuration file for turbotokens".to_string()),
        );
        root.insert(
            "examples".to_string(),
            json!([
                {
                    "$schema": "https://turbotokens.com/config-schema.json",
                    "defaults": {
                        "json": false,
                        "timezone": "Asia/Tokyo",
                        "pricingOverrides": {
...
mod tests {
    ...
    fn agent_configs_expose_only_supported_option_sets() {
        ...
        assert!(schema_property(&schema, &["grok", "defaults", "piPath"]).is_none());
    ...
    fn generated_schema_keeps_legacy_root_definition_shape() {
        ...
        assert_eq!(
            schema["$ref"].as_str(),
            Some("#/definitions/turbotokens-config")
        );
        assert_eq!(
            schema["definitions"]
                .as_object()
        ...
        assert!(
            schema["definitions"]["turbotokens-config"]["properties"]["defaults"]["properties"]
                .is_object()
        );
    }
    ...
    fn schema_allows_repository_example_config() {
        let schema = generated_schema();
        let config =
            serde_json::from_str::<Value>(include_str!("../../../../turbotokens.example.json"))
                .unwrap();
    ...
    fn snapshots_schema_agent_specific_option_edges() {
        ...
        insta::assert_json_snapshot!(json!({
            "rootRef": schema["$ref"],
            "rootProperties": definition_properties(&schema, "turbotokens-config"),
            "rootAdditionalProperties": schema["definitions"]["turbotokens-config"]["additionalProperties"],
    ...
    fn schema_node<'a>(schema: &'a Value, path: &[&str]) -> &'a Value {
        let mut node = &schema["definitions"]["turbotokens-config"];
        for segment in path {
