...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Command::Import(args)) => commands::run_import(args),
        Some(Command::Heatmap(args)) => commands::run_heatmap(args),
        Some(Command::Wrapped(args)) => commands::run_wrapped(args),
...
mod tests {
    ...
    fn shared_usage_types_are_exposed_by_core_crate() {
        assert_eq!(
            std::any::type_name::<turbotokens_core::TokenUsageRaw>(),
    ...
    fn agent_commands_are_exposed_by_independent_crates() {
        let runs: [fn(AgentCommandArgs) -> Result<()>; 17] = [
            ...
            turbotokens_adapter_droid::run,
            turbotokens_adapter_gemini::run,
            turbotokens_adapter_goose::run,
            turbotokens_adapter_grok::run,
            turbotokens_adapter_hermes::run,
            turbotokens_adapter_kilo::run,
            turbotokens_adapter_kimi::run,
            turbotokens_adapter_openclaw::run,
    ...
    fn unified_command_is_exposed_by_independent_crate() {
        let run: fn(AgentCommandArgs) -> Result<()> = turbotokens_adapter_all::run;
    ...
    #[test]
    fn compiled_version_matches_workspace_manifest() {
    ...
    fn formats_windows_user_project_paths_like_typescript() {
        ...
        assert_eq!(
            format_project_name(r"C:\Users\phaedrus\Development\turbotokens", &aliases),
            "turbotokens"
        );
        assert_eq!(
            format_project_name(r"\Users\phaedrus\Development\turbotokens", &aliases),
            "turbotokens"
        );
    }
