...
pub(crate) use daemon_client::try_daily_from_daemon;
...
pub(crate) fn run_daily(args: DailyArgs) -> Result<()> {
    if !args.watch {
        return render_daily_once(&args);
    }
    watch_loop(|| render_daily_once(&args))
}

/// Re-renders a report in place once a second, redrawn from the top-left of
/// the screen so updated rows simply paint over the old frame.
fn watch_loop(render: impl Fn() -> Result<()>) -> Result<()> {
    if !io::stdout().is_terminal() {
        return Err(crate::cli_error("--watch requires a terminal"));
    }
    loop {
...
fn render_daily_once(args: &DailyArgs) -> Result<()> {
    let shared = args.shared.clone();
    // A running daemon serves the same rows in under a millisecond; any
    // incompatibility or error falls back to the direct load path.
    let group_by_project = args.instances || args.project.is_some();
    let mut rows = match try_daily_from_daemon(&shared, args.project.as_deref(), group_by_project) {
        Some(rows) => rows,
        None => load_daily_summaries(&shared, args.project.as_deref(), group_by_project)?,
    };
...
pub(crate) fn run_blocks(args: BlocksArgs) -> Result<()> {
    ...
    if !args.watch {
        return render_blocks_once(&args);
    }
    watch_loop(|| render_blocks_once(&args))
}

fn render_blocks_once(args: &BlocksArgs) -> Result<()> {
    let shared = args.shared.clone();
    let entries = load_entries(&shared, None)?;
...
pub(crate) fn run_live(args: LiveArgs) -> Result<()> {
    match args.agent {
...
fn render_statusline(
    ...
    let session_cost = match args.cost_source {
        ...
        CostSource::Turbotokens => calculate_session_cost(&hook.session_id, shared).ok(),
    ...
    let turbotokens_cost = if args.cost_source == CostSource::Both {
...
/// Context usage is always from the most recent assistant message, which
/// lives near the end of the transcript. Read a tail window first and only
/// fall back to a full read when nothing matches — transcripts grow to tens
/// of megabytes, and statusline runs on every prompt render.
const TRANSCRIPT_TAIL_BYTES: u64 = 256 * 1024;

#[derive(Debug, Deserialize)]
struct TranscriptContextLine {
    #[serde(rename = "type")]
    kind: Option<String>,
    message: Option<TranscriptContextMessage>,
}

#[derive(Debug, Deserialize)]
struct TranscriptContextMessage {
    usage: Option<TranscriptContextUsage>,
}

#[derive(Debug, Deserialize)]
struct TranscriptContextUsage {
    input_tokens: Option<u64>,
    cache_creation_input_tokens: Option<u64>,
    cache_read_input_tokens: Option<u64>,
}

fn latest_context_usage(content: &[u8]) -> Option<u64> {
    let usage_marker = memchr::memmem::Finder::new(br#""usage":{"#);
    for line in crate::fast::byte_lines(content).rev() {
        if memchr::memmem::find(line, br#""type":"assistant""#).is_none()
            || usage_marker.find(line).is_none()
        {
            continue;
        }
        let Ok(line) = serde_json::from_slice::<TranscriptContextLine>(line) else {
            continue;
        };
        if line.kind.as_deref() != Some("assistant") {
            continue;
        }
        let Some(usage) = line.message.and_then(|message| message.usage) else {
            continue;
        };
...
fn calculate_context_tokens_from_transcript(
    ...
    let total_input_tokens = read_tail(path, TRANSCRIPT_TAIL_BYTES)
        .and_then(|tail| latest_context_usage(&tail))
        .or_else(|| latest_context_usage(&fs::read(path).ok()?))?;
    let context_window_size = model_id
        .filter(|model_id| !model_id.is_empty())
        .and_then(|model_id| {
            PricingMap::load_with_overrides(offline, false, shared.pricing_overrides.iter())
...
/// Reads the last `tail_bytes` of `path`, starting at the first newline so
/// the slice begins on a line boundary.
fn read_tail(path: &Path, tail_bytes: u64) -> Option<Vec<u8>> {
    ...
    Read::read_to_end(&mut file, &mut bytes).ok()?;
    if len > tail_bytes
...
mod tests {
    ...
    fn uses_model_context_limit_for_transcript_context_tokens() {
        ...
        shared.pricing_overrides.insert(
            ...
            turbotokens_cli::PricingOverride {
