    ...
    match command {
        ...
        "live" | "stream" => {
            ...
            while parser.peek().is_some() {
                ...
                match parser.next_flag()?.as_str() {
                    "--interval" => {
                        args.interval_ms = parser
                            .value_for("--interval")?
                            .parse()
                            .map_err(|_| "Invalid value for --interval".to_string())?
                    }
                    "--alert-cost" => {
                        args.alert_cost = Some(
                            parser
                                .value_for("--alert-cost")?
                                .parse()
                                .map_err(|_| "Invalid value for --alert-cost".to_string())?,
                        )
                    }
                    "--alert-tokens" => {
                        args.alert_tokens = Some(
                            parser
                                .value_for("--alert-tokens")?
                                .parse()
                                .map_err(|_| "Invalid value for --alert-tokens".to_string())?,
                        )
                    }
                    "--webhook" => args.webhook = Some(parser.value_for("--webhook")?),
                    "--serve" => args.serve = Some(parser.value_for("--serve")?),
                    "--agent" => {
                    ...
                    flag => return Err(format!("Unknown live option '{flag}'")),
                }
            }
            if args.interval_ms == 0 {
                return Err(
                    "Invalid value for --interval '0'. Expected a positive number of milliseconds."
                        .to_string(),
                );
            }
            Ok(Command::Live(args))
        }
        "daemon" => {
            ...
            if let Some(action) = parser
                .peek()
                .filter(|action| matches!(*action, "run" | "start" | "stop" | "status"))
            {
                args.action = match action {
                    ...
                    _ => unreachable!("daemon action is prevalidated"),
                };
                parser.next();
            }
            while parser.peek().is_some() {
                ...
                match parser.next_flag()?.as_str() {
                    ...
                    flag => return Err(format!("Unknown daemon option '{flag}'")),
                }
            }
            Ok(Command::Daemon(args))
        }
        "doctor" => {
        ...
        "completions" => {
            ...
            parser.next();
            if let Some(flag) = parser.peek() {
                return Err(format!("Unknown completions option '{flag}'"));
            }
...
fn report_flag_alias_error(args: &[String]) -> Option<String> {
    ...
    Some(format!(
        "Report flags like {flag} are not supported. Use \"turbotokens {}\" instead.",
...
fn agent_filter_option_error(args: &[String]) -> Option<String> {
    ...
    Some(format!(
        "Agent filters like {flag} are not supported. Use \"turbotokens <agent> <report>\", for example \"turbotokens codex daily\"."
    ))
}
...
fn parse_cost_source(value: &str) -> Result<CostSource, String> {
    match value {
        ...
        "turbotokens" => Ok(CostSource::Turbotokens),
        "cc" => Ok(CostSource::Cc),
