...
fn load_base_rows(
    ...
    let mut specs = vec![
        ...
        AgentLoadSpec {
            ...
            progress_agent: crate::progress::UsageLoadAgent("Grok Build"),
            load: Box::new(|| {
                load_priced_summary_agent_rows(
                    "grok",
                    load_kind,
    ...
    for store in stores {
        ...
        if !collision_owners.is_empty() {
            return Err(crate::cli_error(format!(
                "Invalid turbotokens config: pi.stores name '{}' paths overlap {}",
                store.name,
...
mod tests {
    use super::*;
    use turbotokens_cli::NamedPiStore;
    use turbotokens_test_support::fs_fixture;

    use crate::tests::isolated_agent_env;
    ...
    fn load_rows_filters_named_pi_store_sessions_like_pi_path_subcommand() {
        ...
        let _pi_agent_dir = isolated_agent_env(
            &fixture,
            "PI_AGENT_DIR",
            fixture.path("empty-default").into_os_string(),
        );
    ...
    fn load_rows_rejects_named_pi_store_that_collides_with_default_pi_path() {
        ...
        let _pi_agent_dir = isolated_agent_env(
            &fixture,
            "PI_AGENT_DIR",
            fixture.path(".pi/agent/sessions").into_os_string(),
        );
        let shared = SharedArgs {
            json: true,
            mode: crate::cli::CostMode::Display,
        ...
        assert!(error.to_string().contains("Invalid turbotokens config"));
