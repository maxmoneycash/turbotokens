...
pub struct PricingMap {
    ...
    find_cache: OnceLock<RwLock<FxHashMap<String, Option<Pricing>>>>,
}
...
impl PricingMap {
    ...
    /// Number of model entries currently loaded.
    ...
    pub fn find(&self, model: &str) -> Option<Pricing> {
        ...
        // expensive fuzzy fallback on every call.
        {
            ...
            let guard = cache.read().unwrap_or_else(|error| error.into_inner());
        ...
        let cache = self
            .find_cache
            .get_or_init(|| RwLock::new(FxHashMap::default()));
        let mut guard = cache.write().unwrap_or_else(|error| error.into_inner());
    ...
    fn put_builtin_pricing(&mut self, fast_multiplier_overrides: &FastMultiplierOverrides) {
        ...
        self.entries.insert(
            ...
            Pricing {
                ...
                cache_create_above_200k: Some(4e-6),
                cache_read_above_200k: Some(0.6e-6),
        ...
        self.entries.insert(
            ...
            Pricing {
                ...
                cache_create_above_200k: Some(2e-6),
                cache_read_above_200k: Some(0.4e-6),
                long_context_threshold: Some(200_000),
                fast_multiplier: 1.0,
            },
        );
        ...
        self.context_limits
            .insert("grok-4.20".to_string(), 1_000_000);
        self.context_limits
            .insert("grok-build-0.1".to_string(), 256_000);
        self.context_limits.insert("gpt-5.4".to_string(), 1_050_000);
        // The gpt-5.6 family ships a 922K context window per current upstream
        // pricing data.
        for model in ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"] {
            self.context_limits.insert(model.to_string(), 922_000);
        }
        for model in [
            "claude-opus-4-8",
            "claude-opus-4-7",
...
mod tests {
    ...
    #[test]
    fn embedded_pricing_includes_frontier_models() {
        let pricing = PricingMap::load_embedded();

        assert!(pricing.find("gpt-5.5").is_some());
        assert!(pricing.find("grok-4.3").is_some());
        let grok_45 = pricing.find("grok-4.5").unwrap();
        let grok_420 = pricing.find("grok-4.20").unwrap();
        let grok_build = pricing.find("grok-build-0.1").unwrap();
        assert_eq!(pricing.context_limit("grok-4.3"), Some(1_000_000));
        assert_eq!(pricing.context_limit("grok-4.5"), Some(500_000));
        assert_eq!(pricing.context_limit("grok-4.20"), Some(1_000_000));
        assert_eq!(pricing.context_limit("grok-build-0.1"), Some(256_000));
        assert_eq!(grok_45.cache_read, 0.3e-6);
        assert_eq!(grok_45.input_above_200k, Some(4e-6));
        assert_eq!(grok_420.input, 1.25e-6);
        assert_eq!(grok_420.cache_read, 0.2e-6);
        assert_eq!(grok_420.output, 2.5e-6);
        assert_eq!(grok_build.output_above_200k, Some(4e-6));
    }
    ...
    fn embedded_pricing_includes_gpt_5_6_family_with_long_context_rates() {
        ...
        assert_eq!(pricing.context_limit("gpt-5.6-sol"), Some(922_000));

        let terra = pricing.find("gpt-5.6-terra").unwrap();
        assert_eq!(terra.input, 2e-6);
        assert_eq!(terra.cache_create, 2.5e-6);
    ...
    fn fuzzy_match_allows_date_like_suffixes_for_known_numeric_model_versions() {
        ...
        assert!(pricing.find("claude-opus-4-9").is_none());
        assert!(pricing.find("claude-opus-99").is_none());
    }
